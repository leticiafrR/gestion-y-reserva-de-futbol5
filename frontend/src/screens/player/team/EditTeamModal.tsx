...
import { useUpdateTeam } from "@/services/TeamServices";
...
export const EditTeamModal = ({ team, onClose, onError }: EditTeamModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateTeamMutation.mutateAsync({ teamId: team.id, updates: formData });
      onClose();
    } catch (error: any) {
      onError(error?.message || "Error al actualizar el equipo");
    }
  };
          ...
          <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
            ...
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                fontSize: "14px",
                opacity: updateTeamMutation.isPending ? 0.7 : 1,
              }}
            >
              {updateTeamMutation.isPending ? "Actualizando..." : "Actualizar Equipo"}
            </button>
          </div>
        </form>
      </div>
