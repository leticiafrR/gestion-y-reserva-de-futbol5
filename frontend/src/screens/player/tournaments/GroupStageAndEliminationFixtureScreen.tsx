...
interface GroupStageAndEliminationFixtureScreenProps {
  tournament: Tournament
}

type TabType = "matches" | "standings" | "statistics"

export const GroupStageAndEliminationFixtureScreen = ({ tournament }: GroupStageAndEliminationFixtureScreenProps) => {
  ...
  const getMatchPhase = (matchNumber: number) => {
    ...
    if (matchNumberStr.length > 1 && /^[A-Z]/.test(matchNumberStr)) {
      return `Fase de Grupos - Grupo ${matchNumberStr[0]}`
    }
                      ...
                      <div style={{ 
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                          ...
                          <span style={{ 
                            ...
                            {getMatchPhase(match.matchNumber)}
                          </span>
                    ...
                    <tbody>
                      {standings.map((standing, index) => (
                        <tr key={standing.id.teamId} style={{ 
                          backgroundColor: index < 2 ? "var(--background)" : "transparent",
      ...
      {showGenerateModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "24px", maxWidth: "500px", width: "90%" }}>
            <h3 style={{ fontSize: "20px", fontWeight: "700", marginBottom: "16px" }}>Generar Fixture de Fase de Grupos y Eliminación</h3>
            <p style={{ color: "var(--muted-foreground)", marginBottom: "24px" }}>
              Se generará automáticamente el fixture completo del torneo con formato de{" "}
              <strong>Fase de Grupos y Eliminación</strong> para {tournament.registeredTeams} equipos.
              Los equipos se dividirán en grupos donde jugarán todos contra todos, y los mejores de cada grupo avanzarán a la fase de eliminación.
            </p>
